...
def scrape_metrics(node_exporter_collector_url):
    ...
    try:
        response = requests.get(node_exporter_collector_url, timeout=requests_timeout)
...
async def concurrent_metrics_processing(i, router, router_interfaces, topic):
    """Asynchronous processing of metrics scraping, filtering, and publishing to Kafka."""
...
async def main():
    ...
    while True:
        ...
        for i, router in enumerate(routers.values()):
         # Decide if metrics are scraped from node-exporter-collector or generated through test metrics
            if router["test_metrics"] == False:
                tasks.append(concurrent_metrics_processing(i, router, router_interfaces, kafka_topics[i]))
        ...
        #print(f"{time.time()} - Tiempo total de ejecución con paralelización: {end_time - start_time} segundos.")

        time.sleep(max(0, (polling_interval - (end_time - start_time))))
